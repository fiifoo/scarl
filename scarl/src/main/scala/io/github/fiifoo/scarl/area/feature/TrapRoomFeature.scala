...
case class TrapRoomFeature(bait: ItemSelection,
  ...
  def apply(assets: WorldAssets,
    ...
    val getTrapsMachinery = (this.getTrapsMachinery(assets, context, free - location, location, random) _).tupled
  ...
  private def getTrapsMachinery(assets: WorldAssets,
    ...
    val widget = traps.selection.apply(assets, context, random)

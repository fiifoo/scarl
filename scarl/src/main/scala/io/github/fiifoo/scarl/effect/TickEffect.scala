...
case class TickEffect(target: ActorId,
  ...
  private def regenerate(s: State, creature: Creature, stats: Stats): Option[Effect] = {
    ...
    val energy = if (stats.energy.regen > 0 && creature.resources.energy < stats.energy.max) stats.energy.regen else 0
    val materials = if (stats.materials.regen > 0 && creature.resources.materials < stats.materials.max) stats.materials.regen else 0
